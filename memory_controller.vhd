...
--      7. S_SHOW      : Guarda dato para visualizacion
--      8. S_NEXT      : Avanza a siguiente direccion
--
--   La FSM controla internamente:
--      rom_re, rom_addr, ram_we, ram_re,
--      ram_addr, ram_data_in, display_data
--
...
architecture rtl of memory_controller is
  ...
  -- S_WAIT_ROM  : Primer ciclo de espera (ROM registra direccion)
  -- S_WAIT_ROM2 : Segundo ciclo de espera (dato ROM valido)
  ...
  begin
    if rst = '1' then
      ...
      case state is
        ...
        -- Estado inicial: reinicia el sistema
        when S_IDLE =>
          state <= S_READ_ROM;

        -- Activa rom_re para leer ROM
        -- La ROM registra la direccion en este flanco
        when S_READ_ROM =>
          state <= S_WAIT_ROM;

        -- Primer ciclo de espera
        -- La ROM aun esta procesando la direccion
        -- El dato todavia NO es valido
        when S_WAIT_ROM =>
          state <= S_WAIT_ROM2;

        -- Segundo ciclo de espera
        -- El dato de ROM ya es valido en este ciclo
        ...
        -- Activa ram_re para leer el dato recien escrito
        when S_READ_RAM =>
          state <= S_SHOW;

        -- Captura el dato leido de RAM en display_reg
        -- Este dato se envia a los displays HEX0/HEX1
        when S_SHOW =>
